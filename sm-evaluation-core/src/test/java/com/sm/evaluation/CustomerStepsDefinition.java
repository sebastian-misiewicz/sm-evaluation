...
public class CustomerStepsDefinition {
	...
	@Given("^Customer with the name '(.+)', e-mail '(.+)'$")
	...
	@Given("^Customer with the email '(.+)' opens a shopping cart '(.+)'$")
	public void openShoppingCart(String email, String cartHash) {
	...
	@Given("^Put '(.+)' into the cart '(.+)' in quantity (\\d+)$")
	public void putItemIntoShoppingCart(String name, String cartHash, Integer quantity) {
	...
	@Given("^Checkout the shopping cart '(.+)' with the date '(.+)'$")
	public void checkout(String cartHash, String date) {
		ShoppingCartBE shoppingCartBE = shoppingCartDAO.findOne(cartHash);
